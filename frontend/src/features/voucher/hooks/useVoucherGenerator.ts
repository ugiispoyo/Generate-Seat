...
export function useVoucherGenerator() {
  ...
  const handleGenerate = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setError(
        formatApiError(requestError, "Unable to generate vouchers right now."),
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleRetry = async (index: number) => {
    ...
    try {
      const regeneratedResponse = await reGenerateVoucher({
        seat: index,
        flightNumber: form.flightNumber.trim().toUpperCase(),
      });

      if (!regeneratedResponse.success) {
        setError(regeneratedResponse.message || "Seat retry failed. Please try again.");
        return;
      }

      if (Array.isArray(regeneratedResponse.seats) && regeneratedResponse.seats.length === 3) {
        setSeats(regeneratedResponse.seats);
        return;
      }

      if (regeneratedResponse.seat) {
        setSeats((prevSeats) => {
          const nextSeats = [...prevSeats];
          nextSeats[index] = regeneratedResponse.seat as string;
          return nextSeats;
        });
        return;
      }

      setError("Seat retry failed. Please try again.");
    } catch (requestError: unknown) {
      setError(formatApiError(requestError, "Unable to retry seat right now."));
    } finally {
      setIsLoading(false);
    }
  };
